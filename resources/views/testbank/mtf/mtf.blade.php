                ...
                <tr id="test-question-description">
                    ...
                    <td class="test-body-buttons-column" id="test-bb">
                        <div class="test-body-buttons-column-div">
                            <form method="POST" action="/mtf/{{$test->mtfID}}/publish" class="button-delete-form" @if($test->mtf_items_count == 0) onsubmit="return noItemsPublish();" @else onsubmit="return confirmPublish();" @endif>
                                @csrf
                                @method('PUT')
                                <button class="test-body-buttons @if($test->mtfIsPublic) button-disabled @else buttons-publish-button @endif"><img src="/images/publish-icon-dark.png" class="test-body-buttons-icons" @if($test->mtfIsPublic) disabled @endif>
                                    <p>Publish</p>
                                </button>
                            </form>
                            <button class="test-body-buttons @if($test->mtfIsPublic) button-disabled @else buttons-edit-button @endif" id="test-edit-button" data-id="{{$test->mtfID}}"><img src="/images/edit-icon.png" class="test-body-buttons-icons" @if($test->mtfIsPublic) disabled @endif>
                                <p>Edit</p>
                            </button>
