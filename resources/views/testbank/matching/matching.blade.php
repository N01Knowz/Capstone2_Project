                    ...
                    <td class="test-body-buttons-column" id="test-bb">
                        <div class="test-body-buttons-column-div">
                            ...
                            <form method="POST" action="/matching/{{$test->mtID}}/publish" class="button-delete-form" @if($test->mt_items_count == 0) onsubmit="return noItemsPublish();" @else onsubmit="return confirmPublish();" @endif>
                                @csrf
                                @method('PUT')
                                <button Title="Publish Test" class="test-body-buttons @if($test->mtIsPublic) button-disabled @else button-active @endif" @if($test->mtIsPublic) disabled @endif>
                                    <img src="/images/upload-icon-dark.png" class="test-body-buttons-icons">
                                </button>
                            </form>
                            <button Title="Edit Test" class="test-body-buttons @if($test->mtIsPublic) button-disabled @else buttons-edit-button button-active @endif" @if($test->mtIsPublic) disabled @endif id="test-edit-button" data-id="{{$test->mtID}}">
                                <img src="/images/edit-text-icon-dark.png" class="test-body-buttons-icons">
                            </button>
                            <form method="POST" action="/matching/{{$test->mtID}}" class="button-delete-form" onsubmit="return confirmDelete();">
                                @csrf
                                @method('delete')
                                <button Title="Delete Test" class="test-body-buttons @if($test->mtIsPublic) button-disabled @else button-active @endif" @if($test->mtIsPublic) disabled @endif>
